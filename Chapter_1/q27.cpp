...
struct Flight {
	...
	void deserialize(const std::vector<uint8_t>& bytes) {
		if (bytes.size() != sizeof(Flight)) return;
		memcpy(this, bytes.data(), bytes.size() * sizeof(char_type));
	}
	std::vector<uint8_t> serialize() const {
		std::vector<uint8_t> re; re.resize(sizeof(Flight));
	...
	void read() {
		...
		BUFFER_READ(id, 5);
		out << "Departure date: ";
		departure_date = Date::read();
		out << "Departure time: ";
		departure_hour = Hour::read();
		out << "Location of departure: ";
		BUFFER_READ(port, 20);
		out << "Destination: ";
		BUFFER_READ(dest, 20);
	}
	void write() const {
		out << "Flight ID: " << id << "\nDeparture date: " << departure_date.stringify() << "\nDeparture time: "
			<< departure_hour.stringify() << "\nLocation of departure: " << port << "\nDestination: " << dest << "\n";
	}
};

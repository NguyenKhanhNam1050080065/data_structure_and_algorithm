...
public:
	...
	MixedNumber(const uint32_t& numerator = 0, const uint32_t& denumerator = 0, const uint32_t& integral = 0){
		num = numerator; den = denumerator; in = integral;
	}
	~MixedNumber(){}
	MixedNumber& convert(const double& input) {
...
int main() {
	std::cout << MixedNumber().convert(3.1415926535897932).stringify();

...
public:
	struct HourGroup {
		...
		HourGroup(const uint32_t& hour, const uint32_t& minute){
...
public:
	void set_time(const uint32_t& hour, const uint32_t& minute){
		unix_second = 0;
		if (hour > 24 || minute > 60) return;
		unix_second += hour * HOUR_TO_SECOND;
		unix_second += minute * MINUTE_TO_SECOND;
	}
	Hour(const uint32_t& hour = 0, const uint32_t& minute = 0){
		set_time(hour, minute);
	}
	HourGroup get_hour_group() const {
		uint64_t left = unix_second;
		uint32_t hour = left / HOUR_TO_SECOND;
		left -= hour * HOUR_TO_SECOND;
		uint32_t minute = left / MINUTE_TO_SECOND;
		return HourGroup(hour, minute);
	}
	std::string stringify() const {
		HourGroup hg = get_hour_group();
		return std::string("(") + std::to_string(hg.hour) + ":" + std::to_string(hg.minute) + ")";
	}
	static Hour read() {
		std::string buffer, s_hour, s_min;
		std::getline(std::cin >> std::ws, buffer);
		for (uint32_t i = 0; i < buffer.size(); i++) {
			if (buffer[i] == ':') {
				s_hour.resize(i);
				memcpy(&s_hour[0], &buffer[0], i);
				s_min.resize(buffer.size() - i - 1);
				memcpy(&s_min[0], &buffer[i + 1], buffer.size() - i - 1);
				break;
			}
		}
		return Hour(std::stoi(s_hour), std::stoi(s_min));
	}
};
...
struct MovieTicket {
	...
	void deserialize(const std::vector<uint8_t>& bytes) {
		if (bytes.size() != sizeof(MovieTicket)) return;
		memcpy(this, bytes.data(), bytes.size() * sizeof(int8_t));
	}
	std::vector<uint8_t> serialize() const {
		std::vector<uint8_t> re; re.resize(sizeof(MovieTicket));
		memcpy(re.data(), this, re.size());
		return re;
	}
	void read() {
		std::string buffer;
		std::cout << "Movie's name: ";
		std::getline(std::cin >> std::ws, buffer);
		BUFFER_READ(name, NAMES_SIZE);
		std::cout << "Ticket's price: ";
		std::cin >> price;
		std::cout << "Airing hour: ";
		airing_hour = Hour::read();
		std::cout << "Airing date: ";
		airing_day = Date::read();
	}
	void write() const {
		std::cout << "Movie's name: " << name << "\nTicket's price: " << price << "\nAiring hour: " << airing_hour.stringify() << "\nAiring date: " << airing_day.stringify() << "\n";
	}
};

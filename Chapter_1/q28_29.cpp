...
struct Player {
	...
	void deserialize(const std::vector<uint8_t>& bytes) {
		if (bytes.size() != sizeof(Player)) return;
		memcpy(this, bytes.data(), bytes.size() * sizeof(char_type));
	}
	std::vector<uint8_t> serialize() const {
		std::vector<uint8_t> re; re.resize(sizeof(Player));
...
struct Team {
	...
	Team() {
		memset(name, 0, 30);
		memset(id, 0, 10);
		memset(players, 0, sizeof(Player) * 30);
	}
	void deserialize(const std::vector<uint8_t>& bytes) {
		if (bytes.size() != sizeof(Team)) return;
		memcpy(this, bytes.data(), bytes.size() * sizeof(char_type));
	}
	std::vector<uint8_t> serialize() const {
		std::vector<uint8_t> re; re.resize(sizeof(Team));
		memcpy(re.data(), this, re.size());
		return re;
	}
	void read() {
	...
	void write() const {
		out << "Team's ID: " << id << "\Team's name: " << name << "\nPlayer count: " << player_count << "\n";
		for (uint8_t i = 0; i < player_count; i++){
			players[i].write();
		}
		out << "\n";
